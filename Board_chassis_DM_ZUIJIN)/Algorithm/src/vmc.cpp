...
const float k_m_wheel = 0.266f;//轮的质量
const float k_m_leg=1.149f;
...
//   float phi1_pred = phi1_ + w_phi1_ * predict_dt;  // 预测下一时刻的关节角度(利用关节角速度)
...
//   c0_ = powf(length2, 2) + powf(bd_, 2) - powf(length3 , 2);
//  	float phi2_pred;
//   phi2_pred =2 * atan2f(b0_ - sqrtf(powf(a0_, 2) + powf(b0_, 2) - powf(c0_, 2)), a0_ + c0_);
...
//   v_l0_ = ((sqrtf(powf(x_c_ , 2) + powf(y_c_, 2))) - l0_) / predict_dt;
//   w_theta_ = (phi0_pred - 0.5 * PI - phi_ - theta_) / predict_dt;
//   v_height_ = v_l0_ * arm_cos_f32(theta_) - l0_ * arm_sin_f32(theta_) * w_theta_;
//  dot_v_l0_ = (v_l0_ - last_v_l0_)*2/ (0.002f + 0.008f) + dot_v_l0_ * 0.008f / (0.002f + 0.008f);
//  dotw_theta_ = (w_theta_ - last_w_theta_) *2/ (0.002f + 0.008f) + dotw_theta_ * 0.008f / (0.002f + 0.008f);
...
{
  A1 = (length1*w_phi1_*arm_sin_f32(phi1_-phi3_) + length4*w_phi4_*arm_sin_f32(phi3_-phi4_))/
  arm_sin_f32(phi3_-phi2_);
  dot_xb = -length1*w_phi1_*arm_sin_f32(phi1_);
  dot_yb = length1*w_phi1_*arm_cos_f32(phi1_);
  w_phi0_ = (x_c_*(dot_yb+A1*arm_cos_f32(phi2_)) - y_c_*(dot_xb-A1*arm_sin_f32(phi2_)))/powf(l0_ , 2);
  v_l0_ = (y_c_*(dot_yb+A1*arm_cos_f32(phi2_))+x_c_*(dot_xb-A1*arm_sin_f32(phi2_)))/l0_;
  w_theta_ = w_phi0_ -  phi_;
  v_height_ = v_l0_ * arm_cos_f32(theta_) - l0_ * arm_sin_f32(theta_) * w_theta_;
  //差分计算
  static float predict_dt = 0.001f;
  static float alpha = 0.8f;  // 滤波系数
  dot_v_l0_ = alpha * dot_v_l0_ + (1.0f - alpha) * (v_l0_ - last_v_l0_) / predict_dt;
  dotw_theta_ = alpha * dotw_theta_ + (1.0f - alpha) * (w_theta_ - last_w_theta_) / predict_dt;
  ddot_z_w_ = ddot_z_M_ - dot_v_l0_ * arm_cos_f32(theta_) +
              2.0f * v_l0_ * w_theta_ * arm_sin_f32(theta_) +
              l0_ * dotw_theta_ * arm_sin_f32(theta_) +
             l0_ * powf(w_theta_, 2) * arm_cos_f32(theta_);
